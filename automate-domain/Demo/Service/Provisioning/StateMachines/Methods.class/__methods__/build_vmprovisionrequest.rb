...
begin
  ...
  def get_network(build, matching_options_hash, matching_tags_hash)
    ...
    when 'ManageIQ::Providers::Amazon::CloudManager'
      ...
      # TODO: Cloud subnet depends on cloud_network  scope.  These two need a retool.
      if matching_options_hash[:cloud_subnet].blank?
        # cloud_subnet = provider.cloud_subnets.first
      else
        cloud_subnet = $evm.vmdb(:cloud_subnet).find_by_id(matching_options_hash[:cloud_subnet])
      end
      matching_options_hash[:cloud_subnet] = cloud_subnet.id unless cloud_subnet.blank?
      log(:info, "Build: #{build} - cloud_subnet: #{matching_options_hash[:cloud_subnet]}") unless cloud_subnet.blank?
  ...
  def get_extra_options(build, matching_options_hash, matching_tags_hash)
    ...
    provider = @template.ext_management_system
    case provider.type
    when 'ManageIQ::Providers::Amazon::CloudManager', 'EmsOpenstack';
  ...
  def build_provision_request(build, tags_hash, options_hash)
    ...
    # Skipping this step as we'd like them all to live on in ws_values to work around BZ#1302250 in later state machines.
  ...
  build_sequences.each do |sequence|
    ...
    matching_builds.each do |build|
      ...
      log(:info, "Working through build number " + build.to_s)
      log(:info, "dialogs_options_hash for this build is " + dialogs_options_hash[build].inspect)
      log(:info, "matching_options_hash is " + matching_options_hash.inspect)
