...
begin
  ...
  def add_cloud_network()
    log(:info, "Processing add_cloud_network...", true)
    # 'default' value is chosen in AWS if this value is empty.  So we'll just check if it exists in wsvalues and if so
    # insert that value in this task.
    ws_values = @task.options.fetch(:ws_values, {})
    if ws_values[:cloud_network]
      cloud_network=$evm.vmdb(:cloud_network).find_by_id(ws_values[:cloud_network])
      raise "cloud_network <#{ws_values[:cloud_network]}> does not exist" unless cloud_network
      log(:info, "Found cloud_network <#{cloud_network.id}> with name <#{cloud_network.name}>")
      @task.set_option(:cloud_network, [cloud_network.id, cloud_network.name])
    end
    # If a cloud_subnet was defined, we will need our cloud_network to verify our name and ID.  Otherwise it will use
    # the AWS defined 'default' for that cloud_network
    if ws_values[:cloud_subnet]
      cloud_subnet=cloud_network.cloud_subnets.detect { |cs| cs.id == ws_values[:cloud_subnet].to_i }
      raise "cloud_subnet <#{ws_values[:cloud_subnet]}> does not exist on cloud_network <#{cloud_network.name}>" unless cloud_subnet
      @task.set_option(:cloud_subnet, [cloud_subnet.id, cloud_subnet.name])
    end
    log(:info, "Processing add_cloud_network...Complete", true)
  end
